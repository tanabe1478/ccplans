...
import type { PlanStatus, PlanPriority } from '@ccplans/shared';
import {
  useBulkUpdateStatus,
  useBulkUpdateTags,
  useBulkUpdateAssign,
...
export function BulkActionBar({ totalCount }: BulkActionBarProps) {
  ...
  const isPending = bulkStatus.isPending || bulkTags.isPending ||
    bulkAssign.isPending || bulkPriority.isPending || bulkArchive.isPending;
  ...
  const handleBulkTags = async (action: 'add' | 'remove') => {
    ...
    try {
      ...
      addToast(`Tags ${action === 'add' ? 'added to' : 'removed from'} ${result.succeeded.length} plans`, 'success');
      setTagInput('');
